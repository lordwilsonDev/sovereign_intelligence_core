...
class CrossNodeVerification:
    ...
    def submit_task(self, payload: Dict[str, Any], min_peers: int = 2) -> Dict[str, Any]:
        ...
        task.consensus = self._verify_consensus(task)
        return self._to_dict(task)

    def _dispatch(self, task: TaskAssignment) -> Dict[str, Dict[str, Any]]:
        results: Dict[str, Dict[str, Any]] = {}
        for peer in self.discovery.list_peers():
            node_id = peer.get("node_id")
            if node_id not in task.assigned_to:
                continue
            address = peer.get("address", "127.0.0.1")
            port = int(peer.get("port", 8766) or 8766)
            try:
                resp = requests.post(
                    f"http://{address}:{port}/evolution/run",
                    json=task.payload,
                    timeout=15,
                )
                results[node_id] = {"status": resp.status_code, "body": resp.json() if resp.ok else {"error": resp.status_code}}
            except Exception as exc:
                results[node_id] = {"status": None, "body": {"error": str(exc)}}
        return results

    def _verify_consensus(self, task: TaskAssignment) -> Dict[str, Any]:
        bodies = [r.get("body", {}) for r in task.results.values() if r.get("status") == 200]
        if not bodies:
    ...
    def _to_dict(self, task: TaskAssignment) -> Dict[str, Any]:
        return {
