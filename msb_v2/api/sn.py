...
@router.post("/notify")
def sn_notify(request: NotificationRequest) -> Dict[str, Any]:
...
def sn_status(notification_id: str) -> Dict[str, Any]:
...
@router.post("/ack")
def sn_ack(request: AckRequest) -> Dict[str, Any]:
    result = _engine.ack(request)
    return result or {"status": "not_found"}


@router.get("/templates")
def sn_templates() -> Dict[str, Any]:
    return {"templates": ["job_failed", "daily_summary"]}


@router.get("/preferences")
def sn_preferences() -> Dict[str, Any]:
    return _engine._policy.__dict__


@router.put("/preferences")
def sn_update_preferences(prefs: UserPreferences) -> Dict[str, Any]:
    return {"status": "updated", "preferences": prefs.model_dump()}


@router.get("/history")
def sn_history(limit: int = 50) -> Dict[str, Any]:
